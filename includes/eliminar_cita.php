...
// Verificar si se recibió el ID de la cita a eliminar y asegurarse de que no esté vacío
if (isset($_POST['cita_id']) && !empty($_POST['cita_id'])) {
    // Obtener el ID de la cita a eliminar
    $idCita = mysqli_real_escape_string($conn, $_POST['cita_id']);

    // Consulta SQL para eliminar la cita
    $sql = "DELETE FROM Citas WHERE cita_id = $idCita";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        if ($conn->affected_rows > 0) {
            ...
            echo '<strong>Cita eliminada exitosamente</strong>';
            echo '</div>';
        } else {
            echo '<div class="notification error">';
            echo '<strong>No se encontró ninguna cita con ese ID</strong>';
            echo '</div>';
        }
    } else {
        echo '<div class="notification error">';
        echo '<strong>Error al eliminar la cita: </strong>' . $conn->error;
        echo '</div>';
    }
} else {
    echo '<div class="notification error">';
    echo '<strong>No se recibió el ID de la cita o el campo está vacío</strong>';
    echo '</div>';
}
?>
